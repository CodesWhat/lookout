...
<Callout type="info">
Portwing v0.9.x is a supported pre-v1 release. The controls documented here are implemented and tested, and public interfaces follow the published [stability policy](https://github.com/CodesWhat/portwing/blob/main/STABILITY.md). This page describes the product's security model; it is not a compliance certification or substitute for your own threat model.
</Callout>
...
| Compose logs tail | 500 lines | Clamp the `logs` operation's `--tail`, which is otherwise unbounded |
| Compose combined output | 10 MB | Bound buffered stdout+stderr from a compose subprocess |
| Ed25519 signed-request body | 1 MB | Bound body buffering before signature verification |
| Ed25519 auth body read | 10 s deadline | Stop a slow-trickled body from holding a handler slot before authentication |
...
| Concurrent exec sessions | 100 (edge: fixed; standard: `MAX_EXEC_SESSIONS`, non-positive disables) | Prevent unbounded goroutine growth |
| Concurrent stream sessions | 100 (edge: fixed; standard: `MAX_STREAM_SESSIONS`, non-positive disables). Shared by every proxied streaming Docker request (including `/containers/{id}/logs` with or without `follow`), the adapter `/api/events` SSE route, and follow-mode adapter log reads; non-follow adapter log reads are a single bounded read and take no slot. | Prevent unbounded goroutine growth |
...
The persistent audit sink is disabled by default (`AUDIT_LOG` unset), while the
