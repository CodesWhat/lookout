...
**`api_request`** stream completion:

For Edge Docker response streams, `api_request` is written when the Docker body
ends. A clean EOF records `allowed`; a truncated body or read failure records
`error`, retaining the original HTTP status. `duration_ms` includes the time
spent reading the stream. An active stream has no completion record yet.
