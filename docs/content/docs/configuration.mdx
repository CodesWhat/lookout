---
title: Configuration
description: Complete environment-variable reference for Portwing, grouped by category.
---
...
| `DRYDOCK_URL` | — | WebSocket URL for Edge mode (e.g. `wss://drydock.example.com`). When set, Portwing operates in Edge mode and connects outbound to Drydock. A plaintext `http://`/`ws://` scheme is rejected at startup unless `ALLOW_INSECURE_EDGE_URL=true`. |
...
| `MAX_CLOCK_SKEW_SECONDS` | `60` | Maximum tolerated clock difference (seconds) between client and agent when validating Ed25519 request timestamps. Range: `1..4611686018` seconds. |
...
Duration settings require integer seconds. Empty values use the defaults;
malformed or out-of-range values stop startup and name the setting. The upper
bound also cannot exceed the platform's integer maximum (`2147483647` on
32-bit builds).

| Variable | Default | Description |
|---|---|---|
| `LOG_LEVEL` | `info` | Log verbosity. Accepted values: `debug`, `info`, `warn`, `error`. |
| `HEARTBEAT_INTERVAL` | `30` | Seconds between WebSocket ping frames in Edge mode. The read deadline is `max(2 * HEARTBEAT_INTERVAL, 60)` seconds. Range: `1..4611686018` seconds. |
| `WELCOME_TIMEOUT` | `30` | Seconds to wait for the Drydock welcome message after the WebSocket handshake in Edge mode. Range: `1..4611686018` seconds. |
| `REQUEST_TIMEOUT` | `30` | Timeout in seconds for individual Docker API calls; `0` disables the timeout. Range: `0..4611686018` seconds. |
| `RECONNECT_DELAY` | `1` | Initial reconnect backoff delay in seconds; `0` retries immediately (doubles on each failure, capped at `MAX_RECONNECT_DELAY`, with ±25% jitter). Range: `0..4611686018` seconds. |
| `MAX_RECONNECT_DELAY` | `60` | Maximum reconnect backoff delay in seconds. Range: `0..4611686018` seconds. |
...
| `MAX_STREAM_SESSIONS` | `100` | Standard mode only: maximum concurrent streaming Docker proxy responses. A non-positive value disables the bound. Edge mode enforces the same 100-session limit, but it is fixed and not configurable. |
| `MAX_EXEC_SESSIONS` | `100` | Standard mode only: maximum concurrent hijacked exec/attach sessions. A non-positive value disables the bound. Edge mode enforces the same 100-session limit, but it is fixed and not configurable. |
...
| `AUDIT_BUFFER_SIZE` | `256` | Number of audit records retained in the in-memory ring buffer for pull-based retrieval at `GET /_portwing/audit`. Set to `0` to disable the buffer. Independent of `AUDIT_LOG`. Retained display fields are copied and capped at 4096 bytes for `path`, `stack`, and `container`; 64 bytes for `method`; and 256 bytes for `actor`, `operation`, `exec_id`, and `key_id`. Oversized fields end with `[truncated]` within that cap, preserving UTF-8 character boundaries. These limits apply only to in-memory audit retention, not incoming requests or the persistent audit sink. |
...
| `DD_POLL_INTERVAL` | `300` | Container inventory refresh interval in seconds (Drydock-compatible polling mode). Range: `1..4611686018` seconds. |
