...
`GET /_portwing/health` returns the full readiness body. `GET /health` returns the same shape with `"status": "ok"`, `"ready": false`, and `"docker": "unknown"`, since it does not probe Docker.
...
| `X-Portwing-Signature` + four companion headers | Ed25519 per-request signature |
...
Portwing also exposes a read-only [Model Context Protocol](https://modelcontextprotocol.io/) server at `POST /_portwing/mcp`. It is available in standard mode with any adapter, including `generic`. Edge mode does not register this route — its WebSocket tunnel to Drydock and local operations listener never wire up the MCP handler. AI assistants (Claude, Cursor, Windsurf) can query live container state through this endpoint without issuing raw REST calls.
