...
| `portwing_host_metrics_supported` | gauge | Whether the `/proc`-backed host fields (CPU, memory, network, uptime) could be read (`1`) or not (`0`, no procfs — e.g. native macOS). When `0`, those series are omitted rather than written as zeros. |
...
| `portwing_host_disk_metrics_available` | gauge | Whether host disk usage could be read from the Docker data root (`1`) or not (`0`). Disk reads `statfs` rather than `/proc`, but collection short-circuits when the `/proc` pass fails, so this is never `1` while `portwing_host_metrics_supported` is `0`. When `0`, the disk byte series below are omitted rather than written as zeros. |
...
<Callout type="info">
Disk metrics (`portwing_host_disk_total_bytes`, `portwing_host_disk_used_bytes`) reflect the filesystem that contains the Docker data root, resolved from the daemon's `/info` rather than assumed to be `/var/lib/docker`. They are omitted when the agent is started with disk collection disabled, when the `statfs` read fails or reports an unusable block size, and when the `/proc` collection pass short-circuits first (`portwing_host_disk_metrics_available 0`).
</Callout>
...
| `portwing_auth_failures_total` | counter | `reason` | Total authentication failures, labelled by failure reason: `bad_token`, `no_credentials`, or (for rejected Ed25519 requests) `timestamp-skew`, `replay`, `unknown-key`, `invalid-signature`, `nonce-capacity`. `nonce-capacity` indicates a full cache of unexpired nonces, distinct from replay. |
...
portwing_build_info{version="0.9.16"} 1
...
# HELP portwing_host_metrics_supported Whether host resource metrics are available on this platform (1) or not (0).
# TYPE portwing_host_metrics_supported gauge
portwing_host_metrics_supported 1
...
# HELP portwing_host_disk_metrics_available Whether host disk usage could be read from the Docker data root (1) or not (0).
# TYPE portwing_host_disk_metrics_available gauge
...
```json
{"status":"ok","live":true,"ready":false,"mode":"standard","version":"0.9.16","uptimeSeconds":3842.7,"docker":"unknown","controller":"not_applicable"}
```
...
```json
{"status":"healthy","live":true,"ready":true,"mode":"standard","version":"0.9.16","uptimeSeconds":3842.7,"docker":"connected","controller":"not_applicable"}
```

Edge-mode not-ready response:

```json
{"status":"unhealthy","live":true,"ready":false,"mode":"edge","version":"0.9.16","uptimeSeconds":42.1,"docker":"connected","controller":"disconnected"}
```
