...
<Callout type="info">
Edge Mode is implemented on both sides. Drydock's cross-repo `quality-portwing-fleet-soak.yml` workflow covers it with real Portwing processes under multi-agent reconnect, exec, continuous-log, and controller-backpressure load; Portwing's separate weekly soak covers Standard/generic HTTP and SSE RSS growth. Drydock `v1.6.0-rc.11+` is the minimum for Portwing v0.9.0's complete controller-owned watcher/update behavior. Older 1.6 prereleases and Drydock 1.5 may remain wire-compatible, but do not provide that full feature contract.
</Callout>
...
All authenticated `/api/*` endpoints accept an `Authorization: Bearer`, `X-Dd-Agent-Secret`, or `X-Portwing-Token` header, or a valid Ed25519 request signature in the five `X-Portwing-*` headers. `POST /api/portwing/enroll` is the bootstrap exception when `ENROLLMENT_TOKEN` is configured: it is rate-limited but authenticates with the one-use enrollment token in its request body. The `/health` endpoint is unauthenticated.
...
1. A compatible Drydock controller runs its native Docker watcher and update
   trigger controller-side.
2. Docker inspection and update calls traverse Portwing's authenticated,
   transparent proxy: inbound HTTP in Standard Mode or correlated
   `request`/`response` messages over the outbound WebSocket in Edge Mode.
3. Portwing reports inventory, reads `dd.*` labels, and emits Docker lifecycle
   events. `events: "portwing"` prevents Drydock from opening a second Docker
   event stream through the proxy.
4. Drydock records the watcher result and owns update orchestration and update
   operation events.
...
3. Controller responds with `welcome` containing the poll interval and, on
   controllers that support it, a `capabilities` array the agent negotiates
   against (see below).
4. Portwing sends `dd:component_sync` (watcher/trigger descriptors) before
   `dd:container_sync` (full container inventory), then sends an initial
   `metrics` payload. The ordering lets Drydock establish controller-owned
   update state before it ingests Portwing's raw inventory.
...
The `edge-response-body-b64` value in `hello.capabilities` is negotiated, not
just advertised. A controller that supports it echoes the same value in
`welcome.capabilities`; the agent then carries every non-streaming Docker API
response body as standard base64 in the response message's `bodyBase64` field
(with the legacy `body` field left unset), which is what lets a non-JSON body
such as the plain-text `OK` from `GET /_ping` cross the wire. When the welcome
carries no `capabilities` — any controller before Drydock support landed — the
agent keeps the legacy `body` encoding, so the capability degrades gracefully
without a `portwing/1.0` protocol-version bump.

`edge-request-body-stream` runs the opposite direction, and unlike
`edge-response-body-b64` it is not negotiated against `welcome.capabilities`:
the agent always advertises it in `hello.capabilities`, and that advertisement
is the controller's only signal that it may send a `request` with
`bodyStream: true` and no inline `body`, delivering the body instead as
follow-up `stream` frames carrying base64 chunks plus a terminal `stream_end`,
all sharing the request's `requestId`. The agent reassembles the chunks in
memory, under per-request, aggregate, and concurrency bounds, before
dispatching the request to Docker. A controller that doesn't recognize the
token simply never sets `bodyStream`, so it costs nothing to advertise.
...
| `welcome` | Controller → Agent | Connection accepted; carries `capabilities` on controllers that negotiate them |
...
| `stream_end` | Bidirectional | End of stream, with a `reason` (see below) |
...
On a response stream (an image pull, a build, a `logs?follow`, a container
export, the event stream) the terminal `stream_end` carries one of two
`reason` values, and they are not interchangeable:

| `reason` | Meaning |
|---|---|
| `complete` | The Docker response body ended cleanly at EOF. Every byte the daemon produced is on the wire. |
| `error` | The body ended early: the daemon died mid-stream, a declared `Content-Length` was never reached, or a read failed partway through. The bytes already sent are a prefix, not the whole payload. |

A controller that treats `error` as `complete` will accept a truncated image
layer or tar archive as a finished one, so branch on the value rather than on
the frame's arrival. The agent logs the underlying read error; the wire value
stays a fixed token so it is safe to match on.

The `stream_end` that terminates a controller-to-agent streamed request body
(`bodyStream`, above) is a different direction of the same frame and carries
no `reason`.
...
`exec_output` always carries plain command output, whatever `exec_start` asked
for. `tty: true` (the default when the field is absent) attaches a PTY and
Docker's stream is already raw. `tty: false` gets Docker's stream multiplexing
instead — 8-byte frame headers of `[stream type, 0, 0, 0, big-endian length]`
around each chunk — and the agent strips those before sending, merging stdout
and stderr in arrival order because `exec_output` has no stream field. A
controller must not demultiplex the payload itself. If a non-TTY stream loses
frame sync the agent ends the session with an `exec_end` whose `reason` names
the desynchronization, rather than forwarding bytes it can no longer frame.
...
error body — e.g. `not allowed by portwing preset` or `exec denied:
privileged exec is not allowed` — rather than a bare status code. Drydock's
...
<Callout type="info">
In Edge Mode, a usable positive `pollInterval` from the current controller's
`welcome` overrides the adapter interval. Without a usable override, Portwing
uses the adapter interval, falling back to `DD_POLL_INTERVAL` when the adapter
supplies none. An absent, zero, negative, malformed or out-of-range welcome
interval keeps that fallback and does not close the connection. The previous
connection's override is cleared on reconnect.
</Callout>
...
|---|---|---|
| `GET /api/events` SSE stream | Compatible | `sse.go:ServeHTTP`; `AgentClient.ts:717` |
| `dd:ack` on connect | Compatible | `sse.go:ServeHTTP` (initial `dd:ack` write); `AgentClient.ts:1299` |
...
| `dd:container-added` SSE | Compatible | `sse.go:BroadcastContainerAdded`; `AgentClient.ts:1303` |
| `dd:container-updated` SSE | Compatible | `sse.go:BroadcastContainerUpdated`; `AgentClient.ts:1303` |
...
| `DELETE /api/containers/{id}` returns 204 | Compatible | `routes.go:handleContainerDelete`; `AgentClient.ts:1543` |
...
| `/health` unauthenticated | Compatible | `server/http.go:handleSimpleHealth`; `AgentClient.ts:152` |
| Edge Mode WebSocket `/api/portwing/ws` | Stable (`portwing/1.0`) | Ed25519-only; correlated `request`/`response` messages carry controller-owned watcher/update Docker API calls. Continuous logs require Portwing 0.8+; the real cross-repo gate is Drydock's `quality-portwing-fleet-soak.yml`. |
