...
> [!WARNING]
> **Pre-1.0 software — APIs may still change.** Portwing is pre-`v1.0.0` (currently `v0.9.16`). The compatibility guarantees that already apply are published in [STABILITY.md](STABILITY.md); other surfaces may still change between minor releases. Pin to an exact version and review the [CHANGELOG](CHANGELOG.md) before upgrading.
...
> [!NOTE]
> **v0.9.16 is the current release.** Dependency-only patch: the website, docs and analytics packages move to Next.js 16.3.4, posthog-js 1.427.0, Biome 2.5.12, PostCSS 8.5.28 and refreshed `@types` pins, with the lockfile and the analytics source test synced to match. No binary or protocol changes since v0.9.15; binaries are still rebuilt reproducibly. Full watcher/update feature compatibility requires Drydock `v1.6.0-rc.11+`; the stable wire contract remains `DrydockCompat` 1.4.0. See [CHANGELOG.md](CHANGELOG.md) for the full itemized history.
...
sudo apt install ./portwing_0.9.16_linux_amd64.deb

# Fedora/RHEL (after downloading the matching release asset)
sudo rpm --install ./portwing_0.9.16_linux_amd64.rpm
```
...
services:
  ...
  portwing:
    image: ghcr.io/codeswhat/portwing:0.9.16
    restart: unless-stopped
    depends_on:
      - sockguard
    read_only: true
...
docker run -d \
  ...
  ghcr.io/codeswhat/portwing:0.9.16
```
...
For v0.4.0 and later — including v0.9.16, the current release — see [CHANGELOG.md](CHANGELOG.md) for the full itemized history.
...
TOKEN=$(openssl rand -hex 32)
docker run -e TOKEN="$TOKEN" ... ghcr.io/codeswhat/portwing:0.9.16
```
...
docker run -e TOKEN_HASH="$HASH" ... ghcr.io/codeswhat/portwing:0.9.16
```
...
`portwing_0.9.16_linux_amd64.tar.gz.cyclonedx.json`. The SBOM has no standalone
